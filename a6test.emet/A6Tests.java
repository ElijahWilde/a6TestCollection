...
public class A6Tests {
	...
	/*
	 * Hello. This is Elijah Wilde. 
	 * These tests are tests I used to make sure my code worked but
	 * NOT ALL OF THESE TESTS ARE MINE.
	 * The tests I wrote are "testAllObservablePaintMethods" and 
	 * "PaintMethodsDontNotifyObserversWhenThrowingErrors"
	 * The other tests were taken from the compilation of student a6 tests online.
	 */
	...
	// tests to see if all paint methods notify correctly
	@Test
	public void testAllObservablePaintMethods() {
		ObservablePicture pic = new ObservablePictureImpl(redTenByTen);
		ROIObserverImpl roi = new ROIObserverImpl();
		Region reg = new RegionImpl(0, 0, 9, 9);
		Region reg2 = new RegionImpl(5, 5, 6, 6);
		pic.registerROIObserver(roi, reg);
		
		pic.paint(5, 5, red); // 1
		pic.paint(5, 5, red, 0.5); // 2
		
		pic.paint(5, 5, 5, 5, red); // 3
		pic.paint(5, 5, 5, 5, red, 0.5); // 4
		
		pic.paint(5, 5, 5.0, red); // 5
		pic.paint(5, 5, 5.0, red, 0.5); // 6
			
		pic.paint(5, 5, blueFourBySix); // 7
		pic.paint(5, 5, blueFourBySix, 0.5); // 8

		assertEquals(pic.findROIObservers(reg2)[0].getTimesNotified(), 8);
	}
	
	// checks to see if the paint methods don't notify when throwing an error
	@Test
	public void PaintMethodsDontNotifyObserversWhenThrowingErrors() {
		ObservablePicture pic = new ObservablePictureImpl(redTenByTen);
		ROIObserverImpl roi = new ROIObserverImpl();
		Region reg = new RegionImpl(0, 0, 9, 9);
		Region reg2 = new RegionImpl(5, 5, 6, 6);
		pic.registerROIObserver(roi, reg);
		try {
			pic.paint(-1, 1, red, 1.0);
			fail("Illegal x.");
		} catch (IllegalArgumentException e) {
		}
		try {
			pic.paint(pic.getWidth(), 1, red, 0.5);
			fail("Illegal x.");
		} catch (IllegalArgumentException e) {
		}
		try {
			pic.paint(1, -1, blue, 0.5);
			fail("Illegal y.");
		} catch (IllegalArgumentException e) {
		}
		try {
			pic.paint(1, pic.getHeight(), blue, 0.5);
			fail("Illegal y.");
		} catch (IllegalArgumentException e) {
		}
		try {
			pic.paint(1, 1, green, 1.5);
			fail("Illegal factor.");
		} catch (IllegalArgumentException e) {
		}
		try {
			pic.paint(1, 1, green, -1);
			fail("Illegal factor.");
		} catch (IllegalArgumentException e) {
		}
		...
		assertEquals(pic.findROIObservers(reg2)[0].getTimesNotified(), 1);
